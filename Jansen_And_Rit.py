...
def sigmoid(v, r):
    return max_firing_rate / (
        1 + np.exp(r * (mean_firing_threshold - v))
    )  # output is num_nodes x 1
...
def system_of_equations(x):
    ...
    dx0dt = y0
    dy0dt = (
        A * a * (sigmoid(C2 * x1 - C4 * x2 + C * alpha * calculate_zi(x3), r_0))
        - 2 * a * y0
        - a**2 * x0
    )
    dx1dt = y1
    dy1dt = (
        A * a * (noise + sigmoid(C1 * x0 - C * beta * x2, r_1)) - 2 * a * y1 - a**2 * x1
    )
    dx2dt = y2
    dy2dt = B * b * sigmoid(C3 * x0, r_2) - 2 * b * y2 - b**2 * x2
    dx3dt = y3
    dy3dt = (
        A * ad * (sigmoid(C2 * x1 - C4 * x2 + C * alpha * calculate_zi(x3), r_0))
        - 2 * ad * y3
        - ad**2 * x3
    )
    return [
...
def run_jansen_and_rit(
    ...
    V_T_sim = pass_through_leadfield(
        C2 * x1[-time_points_in_2_secs:]
        - C4 * x2[-time_points_in_2_secs:]
